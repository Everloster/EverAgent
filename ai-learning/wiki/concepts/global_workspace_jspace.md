...
- ~~"点火/ignition"是否真被证明（fig 29 是部分回应还是实质证明）~~ ✅ 2026-07-19 解决：**实质回应但非完整证明**——fig 29 + 附录 figs 70/71（阈值突变、过渡宽度收窄、阈值处双峰）命中 Dehaene 自列的决定性标准；未证：阈下刺激对照、同刺激跨运行分叉、自放大递归。详见报告 45 追问深入节
