...
- **注入位置选择**：LoRA 原论文将矩阵注入 Transformer 注意力的四个权重——Q / K / V / O。论文实验发现：在固定参数预算下，**Q+V 是最佳组合**；**K 投影单独注入的边际效果不显著**，O 投影可选。FFN 层（gate/up/down_proj）非原论文范围，但实践中加上能再获 1-2% 提升。来源：LoRA_深度解析 §应用位置 / §陷阱3
- **典型超参数**：r ∈ {4, 8, 16}，α 通常 = r 或 = 2r，dropout 0.05~0.1。来源：LoRA_深度解析 §关键超参数
- **r 的边际递减**：r=4 到 r=16 通常效果差异很小，r=64 后边际收益极低但参数量翻 4 倍。建议从 r=8 起步。来源：LoRA_深度解析 §陷阱1
