...
- **活跃线（2026-07-20 对话确认）**：
  ...
  - 字节级建模/蒸馏线（paper 47 起：字节级蒸馏突破 Token 天花板——转换方法、缩放律外推可信度、中文场景空白；**paper 47《Breaking the Token Ceiling》精读（09-15，wiki: byte_level_distillation）**）
...
- 2026-09-19：模型效率线新增《DeepSeek-V4.1-Flash 架构解剖：CED 非对称与 KV 压缩极限》（Jabe 派单，虚拟灵枢文章驱动，子代理全文核对 51 页官方技术报告）。增量：①逐条核实自媒体转述，修正五处失真；②架构四组件深读——CED、CSA2、Single-Pass mHC、Engram；③批判压缩口径与 LongBench-V2 长文退化；④四模型对比；⑤修正此前报告的 "high-rank MQA" 术语误植。新建 wiki 概念页 kv_compression_architectures；kv_cache 页开放问题更新。三新问汇入 open-questions。
- 2026-09-22：新增覆盖「System 1 决策模型/校准」线（Laya，微信文章驱动，本机 MPS 实测）；报告：knowledge_reports/Laya_System1决策模型_深度解析_20260922；B 类候选 laya-router-demo（程乐松期 4466 段做广告段识别对照）。
- 2026-09-18：行业观察线新增《Jev 与 System One Models——TypeSafe AI「决策模型」新范式全景研究》（Jabe 派单，EverOwl 快研起点）。增量：①一手榨干官方材料（首页 10 条 FAQ 从 Framer JS 提取、docs 全站、evals 站逐模型数值、jaggedness 缺陷页）——API 三原语（Noul/Choice/Score）、confidence=分布 peakedness 无公式、$0.042/Mtok 输出免费、官方自认"无法证明定价无补贴"；②解构 193.6x/444.6x——两个倍数分别选自不同极端对手（最贵 opus 算成本、最慢 sonnet 算速度），无 ground truth（Astra+Fable 平均当 reference），官方自认 "higher end of real world gains"，准确率叙事应为"sonnet 级精度、1/300 价格"；③"Zero Hallucinations"=构造性零类型错误（Kalai & Vempala 理论同构）但可选错可自信地错、校准宣称零公开证据（Noul 正反不一致 0.72/0.47）；④RLCD 反推=Brier/log score 进 reward（RLCR 先例）[推测]；⑤生态盘点：官方"接口开源模型闭源"10 仓、awesome-jev 90 条目（browser-use 3077 star 为真信号、长尾 SDK 为发布周泡沫）、被 Vercel/框架层吸收而非竞争；⑥HN 1866 分社区质疑 Top5。新建 wiki 概念页 model_calibration，缝合 rlhf/test_time_compute/bitter_lesson。三新问汇入 open-questions。
- 2026-09-17：前沿专题新增《小米 MiMo-V2.6 公开直播 RL 训练：RL Scaling 三维度体系研究》（Jabe 派单，量子位文章驱动）。增量：①一手实抓直播页（真实地址 /rl，文章的 /rl_ 已 404）——成本计数器（两 run 合计 ~$3 万/h，两天 ≈ V3 预训练窄口径 1/5）、train/infer KL 挂牌监控、accepted 超采+partial+prewarm 异步痕迹、batch composition 五类配比；②罗福莉 X 原帖逐字核实（grader compute 挂 test-case+rubric 奖励，承诺数周开源细节）；③纠正报道硬伤"DeepSeek-Flash 74.2%"系 benchmark 名与头部模型分数误拼；④技术深读三路——Fully Async 系统谱系（AReaL/slime/verl/PipelineRL）与 staleness 文献、In-group Credit Assignment 定位（GiGPO 式组内对照 [推测]）、Grader Compute 与 RM overoptimization（Gao et al.）；⑤批判：当前数据支持"正向斜率"不支持"scaling 成立"，给出四条判别清单；"都是算力"锐评漏掉三个人类判断瓶颈。新建 wiki 概念页 rl_scaling，缝合 scaling_laws/test_time_compute/rlhf/agent_harness/bitter_lesson。三新问汇入 open-questions。
...
- `2026-09-11`：行业观察线新增《Anthropic 经济情景模型深度解析》（OpenClaw 转达任务）。精读 Anthropic Institute WP 2026-02（57 页）+ Scenario Explorer：任务实例 CES 模型五参数路径（m×d×a×ψ×ρ），三情景 2030 GDP +1.6/+8.3/+32.4%、劳动份额 60→45.2（extreme）、认知失业率 17.9%；万人调查中位数≈substantial。缝合 CAPEX 泡沫之辩/Intelligence Curse/曾鸣三阶段论。三缺口汇入 open-questions（ε 代理指标/框架更新判别/个人版 ψ 对冲）。
...
- `2026-09-12`：**课程讲义归档进 git**——`scripts/export_openmaic_courses.py` 把 OpenMAIC 7 节课（讲稿+测验含解析）导出为 markdown 落 `ai-learning/courses/`（每节 frontmatter 带课堂 id 可溯源回本地互动版）。学习成果自此同步 GitHub，不再只存本地 `data/classrooms/`。以后每交付新课重跑导出。
