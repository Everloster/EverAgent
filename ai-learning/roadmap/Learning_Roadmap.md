...
> **本文件结构**：上半部分是面向「对话学习模式」的两栏——「核心知识地图」（该掌握的骨架）与「新知识雷达」（领域最新动态）；下半部分「附录：详细课程表」保留原 4 阶段路径正文。
> 分工：成品报告台账（含摘要）见 [CONTEXT.md](../CONTEXT.md)「已有报告」；本文件只放**状态 + 链接**与**计划 backlog**，不复制摘要。

---

## 核心知识地图

> 这门学科该掌握的骨架。每条标 `[已学]` / `[在学]` / `[未学]`，已学的链接到 CONTEXT 对应报告。
> 状态是**派生视图**——出报告后在此翻成 `[已学]`+链接，摘要只写进 CONTEXT，不在此抄录。

### 基础与核心架构
- `[已学]` Transformer / Self-Attention — 见 CONTEXT「论文精读 01」「self_attention 深度解析」
- `[已学]` 残差连接 / CNN（ResNet、AlexNet、ViT、Swin）— 见 CONTEXT 对应精读
- `[未学]` Batch Normalization 专项、VGGNet 专项

### 大语言模型
- `[已学]` 预训练范式（GPT-1/2、BERT、GPT-3、LLaMA 系列）— 见 CONTEXT 对应精读
- `[已学]` Scaling Laws / 涌现能力 — 见 CONTEXT「Scaling_Laws 深度解析」
- `[已学]` 对齐与后训练（InstructGPT、RLHF、Tulu3、推理模型三流派）— 见 CONTEXT 对应报告
- `[已学]` 高效微调与推理（LoRA、KV Cache、推理优化引擎）— 见 CONTEXT 对应报告

### 训练系统与数据工程
- `[已学]` 分布式训练（ZeRO、Megatron-LM、MegaScale、MoE）— 见 CONTEXT 对应报告
- `[已学]` 数据工程（RefinedWeb、LAION-5B）— 见 CONTEXT 对应精读

### 生成模型
- `[已学]` GAN→DDPM→LDM→DiT 演化 — 见 CONTEXT「生成模型演化全景」
- `[未学]` DDIM 加速采样、Consistency Models 专项

### 多模态与前沿
- `[已学]` CLIP / 多模态理解模型对比 / 世界模型 JEPA — 见 CONTEXT 对应报告
- `[已学]` Agent 系统（ReAct、Harness、Memory、自进化）— 见 CONTEXT 对应报告
- `[未学]` 机制可解释性（Mechanistic Interpretability）

---

## 新知识雷达

> 领域最新动态 backlog，是「对话学习模式」下"接下来学什么"的候选。每条带发现日期 + 兴趣匹配 + 优先级。
> 出报告后从此处勾掉，并在「核心知识地图」翻成 `[已学]`。

| 主题 | 发现日期 | 兴趣匹配 | 优先级 | 状态 |
|------|---------|---------|--------|------|
| （待补 — 由对话 / WebSearch 采集前沿动态后填入） | — | — | — | open |

---

# 附录：详细课程表（原 4 阶段路径，保留备查）

> 以下为原始的 4 阶段系统学习路径正文，作为「核心知识地图」的详细背景保留。
