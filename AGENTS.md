...
> 🔗 **两仓结构**：本仓（公开）是知识主体；F 类基础设施与设备家底在**并排的私有仓 `EverAgent-infra`**（`../EverAgent-infra/`）。换机/首次拉取、两仓如何协作 → 先读 [docs/WORKSPACE_SETUP.md](./docs/WORKSPACE_SETUP.md)。
...
| 项目 | 路径 | 内容 |
|------|------|------|
| 开源热点 | `github-trending-analyzer/` | 发 repo 链接 → **默认轻量介绍**（口头讲清，不落文件）；明确要"出报告/深挖/研究"才跑真 repo 研究；trending 汇总 |

**工作流自成体系**（有自己的协议、脚本、命名规范），读该项目 `AGENTS.md` 独立执行。**分流铁律（2026-07-29）**：只丢链接 = 轻量介绍（`gh` + README 口头讲，不出报告）；明确说做任务/出报告/深挖才真跑 repo 研究。
