...
**事A 就是深度研究方法论本身**（4 轮研究 + 7 章中文报告，见 `skills/repo-research/`）。事B 建立在事A 之上：抓榜单 → 对榜上每个 repo 做一次事A → 汇总。

> 报告质量靠**遵循并持续迭代 skill 规范**保证，不靠脚本校验。发现更好的研究做法就直接改 `skills/repo-research/SKILL.md`。
