...
4. **不自动加中文标点**—— 保留原貌，不补
5. **长音频易触发循环幻觉**（两例实证）：① 2026-08-23 硅谷101 E247——无 VAD 时约 39 分钟起整段循环输出无关广告词，音频本身完好；② **2026-09-22 课代表立正 319（transistor 源）——更严重：VAD-only 也在 00:13:01 语音中段起循环（同一句话重复 3225 段、占全文 89.5%）**。教训：**VAD 单独不保险，触发点可以在语音中段**；终极解法 = `--vad -vm <silero> -mc 0`（`-mc 0` 切断上下文自反馈，重转全程无循环）。**质检红线**：转写完必须跑重复段检测（同句连续 >3 段即报警），不能只看开头结尾；`transcribe.py` 默认参数应带 `--vad ... -mc 0`（待改脚本）。
