...
完成 paper_analysis 或 concept_report 后，执行 wiki 更新：

```
1. 识别报告中涉及的科学家、实验室、机构
   → 更新或创建 wiki/entities/{name}.md

2. 识别核心生物学概念（机制、系统、现象）
...
**跨域连接提示**：睡眠科学与 psychology（认知表现）、ai-learning（神经科学启发）的交叉连接记录在 `wiki/concepts/` 页面的 `## 跨域连接` 区块。
