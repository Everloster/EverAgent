...
| `src/grpo_simulation.py` | GRPO 缩尺模拟（2-token toy policy，组内采样 + 规则化奖励 + PPO-clip + KL 惩罚；与 T060 推理模型三大流派报告联动） |
...
exp_008 已包含 GRPO 缩尺模拟脚本（`src/grpo_simulation.py`）、教学笔记（`experiments/exp_008_grpo_simulation.md`）与 `wiki/concepts/grpo.md` 概念页更新（追加 toy 实现细节 + 训练曲线 + sample-reuse 实现陷阱）；exp_008 与 exp_004 同属 GRPO 主题，exp_004 跑真实 3B 模型，exp_008 跑 toy CPU 缩尺，两者互补不互替。后续可扩展为 prompt 条件化版本、PPO baseline 对比、group size 消融实验，但不得重复生成同名 toy GRPO 缩尺模拟。
...
- GRPO vs SFT 效果对比
- 量化（GPTQ/AWQ）效果评估
- GRPO Group size 消融（G=2, 4, 8, 16, 32）
- GRPO toy + prompt 条件化扩展（让 mixed-gt 模式也能 acc → 1.0）
- PPO（带 Critic）vs GRPO（无 Critic）对比（同一 toy 上实现两个 trainer）
