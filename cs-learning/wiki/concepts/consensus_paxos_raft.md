...
> ⚠️ **矛盾标记**："Paxos 是金标准 vs Raft 是替代品"是工程界长期分歧，详见 [overview §4](../overview.md#四技术分歧与未决问题)。
...
## 跨域连接
- [lamport_clocks](./lamport_clocks.md)：共识依赖逻辑时序
- [byzantine_fault_tolerance](./byzantine_fault_tolerance.md)：容错模型扩展到恶意节点
- [coordination_chubby_zk](./coordination_chubby_zk.md)：ZAB ≈ Multi-Paxos 的工程化
- [distributed_storage](./distributed_storage.md)：Spanner Paxos Group / Bigtable Chubby 协调
- [cap_theorem](./cap_theorem.md)：共识协议在 CP 选择下提供强一致
- [two_phase_commit](./two_phase_commit.md)：跨分片事务由 Paxos + 2PC 组合实现
- [computation_theory](./computation_theory.md)：FLP 不可能性（Fischer-Lynch-Paterson 1985）是异步共识的可计算性下界
